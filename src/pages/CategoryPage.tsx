...
export default function CategoryPage() {
  ...
  async function fetchDeleteData(uuid) {
  ...
  const handleOpenMenu = (event, uuid) => {
    setCurrentUUID(uuid)
  ...
  const handleUpdate = async (event, uuid) => {
    event.preventDefault();
    setCurrentCategorie(categories.filter((categorie) => categorie.uuid === uuid)[0])
    toggleModal();
  }

  const handleDelete = async (event, uuid) => {
    event.preventDefault();
